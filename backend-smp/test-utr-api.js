...
const testUTRWithAPI = async () => {
  try {
    const API_BASE = 'http://erpbackend.tarstech.in/api';
    ...
    const testMethods = ['UPI', 'Bank Transfer', 'Card', 'Online'];
    ...
    for (const method of testMethods) {
      ...
      const testData = {
        ...
        transactionId: `${method.toUpperCase()}-001`,
        utr: `${method.toUpperCase()}-UTR-123456`,
        collectedBy: 'Test User'
      };
      ...
      try {
        ...
        // Clean up
        if (methodResponse.data.payment?.receiptNumber) {
