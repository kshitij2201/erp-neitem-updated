...
export default function ApproveOnDutyLeave({ userData }) {
      ...
      try {
        ...
        const endpoint =
          role === "hod"
            ...
            : `http://167.172.216.231:4000/api/leave/principal/${encodeURIComponent(
                department
              )}`;
  ...
  const handleAction = async (leaveId, decision) => {
    ...
    try {
      ...
      const endpoint =
        role === "hod"
          ? `http://167.172.216.231:4000/api/leave/hod/${leaveId}`
          : `http://167.172.216.231:4000/api/leave/principal/${leaveId}`;
