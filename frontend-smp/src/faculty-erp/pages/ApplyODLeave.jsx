  ...
  const fetchSubmittedLeaves = async () => {
    ...
    try {
      ...
      const [regularLeavesResponse, odLeavesResponse] = await Promise.all([
        axios.get(
          `http://167.172.216.231:4000/api/leave/my-leaves/${employeeId}`,
          {
            headers: {
        ...
        axios.get(
          `http://167.172.216.231:4000/api/leave/my-od-leaves/${employeeId}`,
          {
            headers: {
              Authorization: `Bearer ${userData.token}`,
            },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://167.172.216.231:4000/api/leave/odleave/apply",
        formData,
        {
          headers: {
            Authorization: `Bearer ${userData.token}`,
