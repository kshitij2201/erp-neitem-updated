    ...
    try {
      ...
      if (userRole.toLowerCase() === "hod") {
        ...
        const response = await axios.get(
          `http://erpbackend.tarstech.in/api/leave/hod/${encodeURIComponent(
            userDepartment
          )}`,
          {
        ...
        const response = await axios.get(
          `http://erpbackend.tarstech.in/api/leave/principal/all`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const response = await axios.get(
          "http://erpbackend.tarstech.in/api/leave/all",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
      ...
      if (userRole.toLowerCase() === "hod") {
        // HOD should see ALL OD requests from their department (all statuses)
        const receivedData = allLeaves.filter((leave) => {
          const matchesDepartment = leave.department === userDepartment;
          const isODLeave = leave.leaveCategory === "OD";

          console.log("Checking OD leave:", {
            id: leave._id,
          ...
          return matchesDepartment && isODLeave;
        });
        console.log("HOD received OD requests (all statuses):", receivedData);
        console.log("Total OD leaves fetched:", allLeaves.length);
        console.log("Filtered OD for HOD:", receivedData.length);
    ...
    try {
      ...
      const endpoint =
        userRole.toLowerCase() === "hod"
          ? `http://erpbackend.tarstech.in/api/leave/hod/${id}`
          : `http://erpbackend.tarstech.in/api/leave/principal/${id}`;
                ...
                receivedRequests.map((request) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                      {/* Request header with department, ID, and action indicator */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-900 ml-6">
                          <div>
                            <strong>Event:</strong> {request.eventName || "N/A"}
                          </div>
                          <div>
                            <strong>Location:</strong>{" "}
                            {request.location || "N/A"}
