  ...
  const fetchRequests = async () => {
    ...
    try {
      const response = await axios.get(
        "http://erpbackend.tarstech.in/api/leave/odleave/all",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      const allODLeaves = response.data || [];
      console.log("Fetched OD leaves:", allODLeaves);
      ...
      if (userRole.toLowerCase() === "hod") {
        ...
        const receivedData = allODLeaves.filter(
          (leave) =>
      ...
      const sentData = allODLeaves.filter(
        (leave) =>
                      ...
                      <div>
                        <div className="mb-6">
                          ...
                          <div className="text-sm text-gray-900 ml-6">
                            <div>
                              <strong>Event:</strong>{" "}
                              {request.odDetails?.eventName || "N/A"}
                            </div>
                            <div>
                              <strong>Location:</strong>{" "}
                              {request.odDetails?.location || "N/A"}
                            </div>
                          </div>
                        </div>
