...
const BookActions = () => {
    ...
    try {
      ...
      if (type === "faculty") {
        ...
        try {
          ...
          try {
            const response = await axios.get(
              `localhost:4000/api/students/enrollment/${encodeURIComponent(
                studentId
              )}`,
              { headers: getAuthHeaders() }
            );
  ...
  const fetchBookDetails = async (accno) => {
    try {
      ...
      try {
        ...
        const response = await axios.get(
          `localhost:4000/api/books/by-accno/${accno}`,
          { headers: getAuthHeaders() }
        );
        ...
        try {
          ...
          const response = await axios.get(`localhost:4000/api/books/${accno}`);
          bookData = response.data;
          ...
          try {
            // Try search endpoint
            const response = await axios.get(
              `localhost:4000/api/books?search=${accno}`
            );
            const books =
            ...
            try {
              ...
              const response = await axios.get(`localhost:4000/api/books`);
              const allBooks =
                response.data.data || response.data.books || response.data;
  ...
  const saveNewBook = async () => {
    try {
      ...
      if (response.data) {
        ...
        try {
          await axios.patch(
            `localhost:4000/api/books/accno/${formData.ACCNO}`,
            {
              status: "available",
              QUANTITY: 1,
            }
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (response.data.data.records.length === 0) {
        ...
        if (filters.ACCNO) {
          try {
            await axios.get(`localhost:4000/api/books/accno/${filters.ACCNO}`);
          } catch (err) {
            if (err.response?.status === 404) {
  ...
  const searchBooksByACCNO = async (accno) => {
    const response = await fetch(`localhost:4000/api/books?accno=${accno}`);
    const books = await response.json();
    return books;
  };
