  ...
  const fetchBookDetails = async (accno) => {
    try {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:4000/api/books/by-accno/${accno}`,
          { headers: getAuthHeaders() }
        );
        ...
        try {
          ...
          const response = await axios.get(
            `http://localhost:4000/api/books/${accno}`
          );
          bookData = response.data;
          ...
          try {
            // Try search endpoint
            const response = await axios.get(
              `http://localhost:4000/api/books?search=${accno}`
            );
            const books =
  ...
  const saveNewBook = async () => {
    try {
      ...
      if (response.data) {
        ...
        try {
          await axios.patch(
            `http://localhost:4000/api/books/accno/${formData.ACCNO}`,
            {
              status: "available",
              QUANTITY: 1,
            }
    ...
    try {
      ...
      if (response.data.data.records.length === 0) {
        ...
        if (filters.ACCNO) {
          try {
            await axios.get(
              `http://localhost:4000/api/books/accno/${filters.ACCNO}`
            );
          } catch (err) {
            if (err.response?.status === 404) {
