...
const RoleLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // If it's Bus Management, also store user data for admin dashboard access
      if (faculty.designation.trim() === "Bus Management") {
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            name: faculty.name,
            email: faculty.email,
            role: "admin", // Set role as admin for bus management access
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          {error && (
            <div
              className="bg-red-50 border border-red-200 rounded-md p-3 flex items-center text-red-600 text-sm"
              role="alert"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="mt-4 text-center text-xs text-gray-500">
          <p>For faculty management access only</p>
        </div>
      </div>
    </div>
  );
