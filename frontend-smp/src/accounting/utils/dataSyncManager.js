...
class DataSyncManager {
  ...
  async syncSalaryToPF(salaryData) {
    try {
      const response = await fetch(
        `https://backenderp.tarstech.in/api/faculty/employee/${salaryData.name}/auto-generate-pf`,
        {
          method: "POST",
  ...
  async syncPFToIncomeTax(pfData) {
    try {
      ...
      const existingResponse = await fetch(
        `https://backenderp.tarstech.in/api/income-tax?employeeName=${pfData.employeeName}`
      );
      ...
      if (existing.length > 0) {
        ...
        await fetch(
          `https://backenderp.tarstech.in/api/income-tax/${existing[0]._id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
        ...
        // Create new record
        await fetch("https://backenderp.tarstech.in/api/income-tax", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async initializeStudentFees(studentData) {
    try {
      ...
      const response = await fetch(
        `https://backenderp.tarstech.in/api/fee-heads/applicable/${studentData._id}`
      );
  ...
  async getSystemHealth() {
    try {
      const endpoints = [
        "https://backenderp.tarstech.in/api/students/stats/overview",
        "https://backenderp.tarstech.in/api/faculty/status",
        "https://backenderp.tarstech.in/api/students/fees/status",
      ];
  ...
  async performFullSync() {
    ...
    try {
      ...
      const pfResponse = await fetch("https://backenderp.tarstech.in/api/pf");
