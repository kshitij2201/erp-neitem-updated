...
class DataSyncManager {
  ...
  async syncSalaryToPF(salaryData) {
    try {
      const response = await fetch(
        `http://erpbackend.tarstech.in/api/faculty/employee/${salaryData.name}/auto-generate-pf`,
        {
          method: "POST",
  ...
  async syncPFToIncomeTax(pfData) {
    try {
      ...
      const existingResponse = await fetch(
        `http://erpbackend.tarstech.in/api/income-tax?employeeName=${pfData.employeeName}`
      );
      ...
      if (existing.length > 0) {
        ...
        await fetch(
          `http://erpbackend.tarstech.in/api/income-tax/${existing[0]._id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
        ...
        // Create new record
        await fetch("http://erpbackend.tarstech.in/api/income-tax", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async initializeStudentFees(studentData) {
    try {
      ...
      const response = await fetch(
        `http://erpbackend.tarstech.in/api/fee-heads/applicable/${studentData._id}`
      );
  ...
  async getSystemHealth() {
    try {
      const endpoints = [
        "http://erpbackend.tarstech.in/api/students/stats/overview",
        "http://erpbackend.tarstech.in/api/faculty/status",
        "http://erpbackend.tarstech.in/api/students/fees/status",
      ];
  ...
  async performFullSync() {
    ...
    try {
      ...
      const pfResponse = await fetch("http://erpbackend.tarstech.in/api/pf");
