...
export default function SalarySlip() {
  ...
  useEffect(() => {
    // Fetch faculty list with proper authentication
    ...
    fetch("http://erpbackend.tarstech.in/api/faculty", {
      headers: authHeaders,
    })
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          ...
          console.log("No faculty data received from API");
          setFaculty([]);
        }
      })
      .catch((err) => {
  ...
  // Automatically print salary slip when it's generated and show notification
  // Automatically scroll to the salary slip and print when generated
  useEffect(() => {
    if (salarySlip) {
      // Show success message
      setSuccessMessage("Salary slip generated successfully!");

      // Scroll to the salary slip section
      const salarySlipElement = document.getElementById("salary-slip-section");
      if (salarySlipElement) {
        salarySlipElement.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }

      // Wait a moment to ensure the state is fully updated and UI has rendered
      setTimeout(() => {
        printSalarySlip();
      }, 800);
    }
  ...
  // Fetch salary records for selected faculty and year
  useEffect(() => {
    if (!selectedFaculty) {
      setSalaryRecords([]);
      return;
    ...
    fetch(
      `/api/faculty/salary?name=${encodeURIComponent(
  ...
  // Function to directly fetch income tax data for the selected faculty
  const fetchIncomeTaxData = async (facultyName) => {
    try {
      setLoading(true);
      // First try to fetch from direct income tax endpoint
      const response = await fetch(
        `/api/income-tax?employeeName=${encodeURIComponent(facultyName)}`
      );

      if (response.ok) {
        const data = await response.json();
        if (Array.isArray(data) && data.length > 0) {
          // Sort by financial year to get the most recent record
          data.sort((a, b) => {
          ...
          console.log("Found income tax data:", latestRecord);
          setIncomeTaxData(latestRecord);

          // Calculate monthly tax deduction
          const annualTaxAmount = parseFloat(latestRecord.taxLiability || 0);
          const monthlyDeduction = annualTaxAmount / 12;
      ...
      // If no direct income tax data, return null
  ...
  const generateSalarySlip = async () => {
    ...
    try {
      ...
      // Find the salary record for the selected month
      const monthStr = selectedMonth.toString().padStart(2, "0");
      const formattedMonth = `${selectedYear}-${monthStr}`;
      const salaryRecord = salaryRecords.find(
        (rec) => rec.month === formattedMonth
      );

      // If no salary record is found in database, we'll use faculty data
      // to create an estimated salary slip
      const isEstimatedSlip = !salaryRecord;

      // Get employee profile data which includes PF info
      let profileData = {};
      try {
        const profileResponse = await fetch(
          `/api/faculty/employee/${encodeURIComponent(
            facultyMember.personalInfo.fullName
          )}/profile`
        );
        if (profileResponse.ok) {
          ...
          console.warn(
            "Could not fetch employee profile, using default values"
          );
        }
      } catch (err) {
        console.warn("Error fetching employee profile:", err);
        // Continue with empty profile data rather than failing completely
      }

      // Combine salary record with faculty and profile data
      let basicSalary = facultyMember.salaryInfo?.basicSalary || 0;
      let allowances = facultyMember.salaryInfo?.allowances || {};
      let deductions = facultyMember.salaryInfo?.deductions || {};

      // Use real deduction values from PF records if available
      if (profileData.pf) {
        deductions.pf =
          profileData.pf.employeeContribution || deductions.pf || 0;
      }

      // Fetch income tax data for the selected employee
      try {
        const incomeTaxResponse = await fetch(
          `/api/income-tax?employeeName=${encodeURIComponent(
            facultyMember.personalInfo.fullName
          )}`
        );
        if (incomeTaxResponse.ok) {
          const incomeTaxData = await incomeTaxResponse.json();
          if (Array.isArray(incomeTaxData) && incomeTaxData.length > 0) {
            // Get the most recent income tax record
            const latestIncomeTax = incomeTaxData.sort((a, b) => {
              const yearA = parseInt(a.financialYear.split("-")[0]);
              const yearB = parseInt(b.financialYear.split("-")[0]);
              return yearB - yearA;
            })[0];

            // Calculate monthly income tax deduction
            const annualTax = parseFloat(latestIncomeTax.taxLiability || 0);
            const monthlyIncomeTax = annualTax / 12;
            deductions.incomeTax = parseFloat(monthlyIncomeTax.toFixed(2));
          }
        }
      } catch (error) {
        console.error("Error fetching income tax data:", error);
      ...
      // First convert all values to numbers to prevent string concatenation
      ...
      // Determine gross salary from best available source
      let grossSalary = 0;

      if (salaryRecord && salaryRecord.amount !== undefined) {
        // If we have a salary record from the database, use that
        grossSalary = parseFloat(salaryRecord.amount.toFixed(2));
      } else {
        // Otherwise calculate from the component values
        grossSalary = basicSalaryNum + totalAllowances;
      }

      // Convert all deduction values to numbers
      ...
      let totalDeductions =
        sumDeductions > 0 ? parseFloat(sumDeductions.toFixed(2)) : 0;

      // Calculate net salary
      let netSalary = parseFloat((grossSalary - totalDeductions).toFixed(2));

      // Create the final salary slip object with all calculated values
      const slip = {
        ...
        isEstimated: isEstimatedSlip,
      };

      // Clear any previous error messages before setting the new slip
      setError(null);
      setSalarySlip(slip);

      // Provide additional success feedback
      setSuccessMessage("Salary slip generated successfully!");
    } catch (err) {
  ...
  const printSalarySlip = () => {
    ...
    if (!printWindow) {
      setError("Please allow pop-ups to print the salary slip");
      return;
    }
        ...
        <body>
          <div class="slip-container">
            <div class="document-id">
              Doc ID: SS-${salarySlip.faculty.employeeId}-${salarySlip.month
      .substring(0, 3)
                ...
                <table class="salary-table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="amount">₹${salarySlip.totalDeductions.toLocaleString()}</td>
                    </tr>
                    <tr class="net-salary">
                ...
                <div class="bank-grid">
                  ...
                  <div class="bank-item">
                    ...
                    <span class="bank-value">${
                      ...
                      "XXXX-XXXX-" +
                        (salarySlip.faculty.employeeId || "0000").slice(-4)
                    }</span>
                  </div>
                  <div class="bank-item">
  ...
  const generateReceipt = () => {
        ...
        <body>
          <div class="receipt-container">
            ...
            <div class="receipt-header">
              <div class="header-border"></div>
              <div class="receipt-no">Receipt #: NIETM-SAL-${
                salarySlip.faculty.employeeId
      ...
      salarySlip.year
            ...
            <div class="receipt-footer">
              ...
              <div class="footer-note">
                ...
                For any discrepancies, please contact the Finance Department.
              </div>
            </div>
          </div>
        </body>
    ...
    setTimeout(() => {
      try {
        printWindow.print();
        setSuccessMessage(
          "Print dialog opened. Please complete the printing process."
        );
      } catch (err) {
        console.error("Print error:", err);
        setError(
          "Failed to open print dialog. Please try again or check your browser settings."
        );
      }
    }, 800);
  ...
  // Show full page loading overlay when initially loading faculty data
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              {faculty.map((f) => (
                <option key={f._id} value={f._id}>
                  {`${f.personalInfo?.firstName || ""} ${
                    f.personalInfo?.fathersName || ""
                  } ${f.personalInfo?.lastName || ""}`.trim()}{" "}
                  ({f.employeeId})
                  {/* {f.personalInfo?.fullName} ({f.employeeId}) */}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-end">
            ...
            >
              {loading ? (
                <>
                  <LoadingIndicator size="sm" message="" type="spinner" />
                  Generating...
                </>
              ) : (
                <>
      ...
      {salarySlip && (
        <div
          id="salary-slip-section"
          className="bg-white rounded-lg shadow-lg p-6 relative"
        >
          {/* Highlight effect */}
          <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-green-400/30 to-blue-500/30 animate-pulse pointer-events-none"></div>
