...
const IncomeTax = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      const response = await fetch(
        "http://167.172.216.231:4000/api/income-tax/stats/dashboard"
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchTaxSlabs = async () => {
    try {
      const response = await fetch(
        "http://167.172.216.231:4000/api/income-tax/info/tax-slabs"
      );
      if (response.ok) {
        const data = await response.json();
        setTaxSlabs(data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingRecord
        ? `http://167.172.216.231:4000/api/income-tax/${editingRecord._id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        const response = await fetch(
          `http://167.172.216.231:4000/api/income-tax/${id}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch(
        `http://167.172.216.231:4000/api/income-tax/export/csv?${params}`
      );
      if (response.ok) {
