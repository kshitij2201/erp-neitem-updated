...
const FacultyDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch(
        `http://142.93.177.150:4000/api/faculty/dashboard/data?financialYear=${selectedFY}`,
        {
          headers: {
  ...
  const fetchStatusData = async () => {
    try {
      ...
      const response = await fetch(
        `http://142.93.177.150:4000/api/faculty/dashboard/status?financialYear=${selectedFY}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleEmployeeClick = async (employeeName) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://142.93.177.150:4000/api/faculty/employee/${encodeURIComponent(
          employeeName
  ...
  const handleBulkGeneratePF = async () => {
    if (
      ...
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          "http://142.93.177.150:4000/api/faculty/bulk-operations",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
