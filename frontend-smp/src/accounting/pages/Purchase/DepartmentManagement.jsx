...
const DepartmentManagement = () => {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      const response = await fetch(
        "http://erpbackend.tarstech.in/api/purchase/departments"
      );
      if (!response.ok) {
  ...
  const fetchFaculty = async () => {
    try {
      const response = await fetch(
        "http://erpbackend.tarstech.in/api/purchase/faculty"
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingDepartment
        ? `http://erpbackend.tarstech.in/api/purchase/departments/${editingDepartment._id}`
        : "http://erpbackend.tarstech.in/api/purchase/departments";
  ...
  const handleDelete = async (departmentId) => {
    if (window.confirm("Are you sure you want to delete this department?")) {
      try {
        const response = await fetch(
          `http://erpbackend.tarstech.in/api/purchase/departments/${departmentId}`,
          {
            method: "DELETE",
          }
        );
