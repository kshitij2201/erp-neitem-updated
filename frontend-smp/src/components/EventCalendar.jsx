...
const EventCalendar = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(
        "http://167.172.216.231:4000/api/superadmin/events",
        {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://167.172.216.231:4000/api/superadmin/events/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `http://167.172.216.231:4000/api/superadmin/events/${editingId}`,
        {
          ...editedEvent,
